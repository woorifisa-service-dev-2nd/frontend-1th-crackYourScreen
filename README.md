...
3.그 다음 원래 크기로 바꾸는 작업을 한다. 이때 함수에 두 개의 인수를 줘서 진행하는데 하나는 원래 크기로 돌아가기 위한 설정 값이고 또 다른 값은 돌아가기 까지의 시간을 의미합니다. 원래 크기는 50px로 설정했고 200ms시간으로 설정했기 때문에 0.2초 후에 원래 크기 50px가 된다.
ex)
setTimeout(() => {
